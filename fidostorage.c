...
mbed_error_t    fidostorage_configure(uint8_t *buf, uint16_t  buflen, uint8_t *aes_key)
{
    ...
    uint32_t keylen = AES_KEY_LEN;
    if ((errcode = fidostorage_get_hmac_key_from_master(aes_key, &ctx.hmac_key[0], &keylen)) != MBED_ERROR_NONE) {
...
mbed_error_t    fidostorage_get_appid_slot(uint8_t const * const appid, uint32_t *slot, uint8_t *hmac)
{
    ...
    while (curr_sector <= SLOT_NUM/8) {
        ...
        for (uint16_t j = 0; j < numcell; ++j) {
            ...
            hmac_update(&hmac_ctx, &appid_table[j].appid[0], sizeof(fidostorage_appid_table_t));
        }
        curr_sector++;
    }
                                               ...
                                               fidostorage_appid_slot_t *data_buffer)
{
    ...
    if ((errcode = read_encrypted_SD_crypto_sectors(&ctx.buf[0], 8*SECTOR_SIZE, appid_slot/8)) != MBED_ERROR_NONE) {
        log_printf("[fidostorage] Failed during SD_enc_read, from sector %d: ret=%d\n", appid_slot/8, errcode);
        errcode = MBED_ERROR_RDERROR;
        goto err;
    }
    ...
    log_printf("|--> appid: ");
    hexdump(mt->appid, 32);
    log_printf("|--> flags:      %x", mt->flags);
    log_printf("|--> name:       %s", mt->name);
    log_printf("|--> CTR:        %ld", mt->ctr);
    log_printf("|--> icon_len:   %ld", mt->icon_len);
err:
#if CONFIG_USR_LIB_FIDOSTORAGE_PERFS
    sys_get_systick(&ms2, PREC_MILLI);
